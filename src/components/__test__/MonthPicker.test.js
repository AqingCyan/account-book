...
describe("test MonthPicker component", () => {
  ...
  it("after click the button, dropdown should show, year list&month list should have the correct items", () => {
    ...
    // the first year should be the correct year minus 4
    expect(
      wrapper
  ...
  it("click the year&month item, should trigger the tight status change", () => {
    wrapper.find(".dropdown-toggle").simulate("click")
    wrapper
      .find(".years-range .dropdown-item")
      .first()
  ...
  it("after the dropdown is shown, click the document should close the dropdown", () => {
    ...
    expect(wrapper.state("isOpen")).toEqual(true)
    expect(wrapper.find(".dropdown-menu").length).toEqual(1)
    // 测试点击 dropdown 其他区域致使 dropdown关闭
    eventMap.click({
